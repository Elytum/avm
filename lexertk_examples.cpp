...
int main()
{
   ...
   (void)program;
   // const std::string test = std::ifstream in("./samples/subject.avm");
   std::string s((std::istreambuf_iterator<char>(std::ifstream in("./samples/subject.avm"))), std::istreambuf_iterator<char>());
   std::cout << s << std::endl;
   // interprete(program);
   return 0;
}
